...
Administrators can now turn member OTP on or off in Settings → Member OTP, in any election phase. The saved database setting overrides config.local.php; before a choice is saved, the existing configuration/default applies (off for testing). Resets preserve this setting. Successful OTP verification resets that email’s hourly request counter and resend cooldown, and clears the successfully used code’s attempt count. The code remains consumed and cannot be reused. Shared-network limits remain separate; incorrect, expired, and locked codes do not trigger resets.

### Manual tie-break vote
Export the original results and record already elected members before resetting. Use Reset nominations and votes (retain members), then Nominations → Manual nominees for a tie-break vote. Select at least two tied Members and the remaining positions to fill. Preparing the runoff adds accepted nominees with zero nomination votes, keeps member photos, and enters review. Click Open voting to start; the vote limit and officer count equal the remaining positions. Officers cannot be selected. The new results cover the runoff only; earlier winners are not automatically combined.
