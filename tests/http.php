...
try {
    ...
    [, $toggleHtml]=post('admin','index.php?page=settings',['action'=>'toggle_member_otp','enabled'=>'0']);
    check(str_contains($toggleHtml,'Member OTP is now off') && $pdo->query('SELECT member_otp_enabled FROM elections')->fetchColumn()==0,'Admin can turn member OTP off during nominations');
    ...
    [, $toggleHtml]=post('admin','index.php?page=settings',['action'=>'toggle_member_otp','enabled'=>'1']);
    check(str_contains($toggleHtml,'Member OTP is now on') && $pdo->query('SELECT member_otp_enabled FROM elections')->fetchColumn()==1,'Admin can restore OTP without editing configuration');
    ...
    post('member',$nom,['action'=>'verify_otp','code'=>$code==='000000'?'000001':'000000']);
